...
export namespace DAO {
  ...
  export class Discover {
    ...
    static async getAll(): Promise<Array<DiscoverUser>> {
      var query = `SELECT Users.show_on_discover, Users.id, Users.email, Business.industry, Business.company_name as username, Business.logo, Business.description, Business.status FROM Users JOIN Business on Users.id = Business.user_id WHERE show_on_discover = 1 UNION ALL SELECT Users.show_on_discover, Users.id, Users.email, Individual.industry, Individual.name as username, Individual.profile_picture AS logo, Individual.bio AS description, Individual.status FROM Users JOIN Individual on Users.id = Individual.user_id WHERE show_on_discover = 1;`
      var [results] = await connection.promise().query(query)
  ...
  export class Business {
    ...
    static async getByUserId(userId: number): Promise<Business_Type | boolean> {
      var query = `SELECT b.*, u.is_subscribed, u.email FROM Business b, Users u WHERE u.id = b.user_id and b.user_id=?;`
      var [result] = await connection.promise().query(query, [userId])
     ...
     * @param {number} occupation The new industry of the business (a number relating to a specific label)
    ...
    static async update(
      ...
      var query = `UPDATE Business INNER JOIN Users ON Users.id = Business.user_id SET Business.company_name = ?, Business.logo = ?, Business.description = ?, Business.location = ?, Business.industry = ?, Business.occupation = ?, Business.size = ?, Business.website = ?, Business.status = ?, Users.is_subscribed = ? WHERE Business.user_id = ?;`
      await connection
        .promise()
        .execute(query, [
          name,
  ...
  export class Individual {
    ...
    static async getByUserId(
      ...
      var query = `SELECT i.*, u.is_subscribed, u.email FROM Individual i, Users u WHERE u.id = i.user_id and i.user_id=?;`
      var [result] = await connection.promise().query(query, [userId])
  ...
  export class Network {
    static async getInvitesByUserId(id: number): Promise<Array<NetworkInvite>> {
      var query = `SELECT * FROM network_invites WHERE accepted = 0;`
      var [results] = await connection
        .promise()
        .query(query)

      const result = (results as Array<RowDataPacket>).filter(a => a.requestor_user_id == id || a.invitee_user_id == id).map(
        (value: RowDataPacket) => {
          return {
            ...value,
